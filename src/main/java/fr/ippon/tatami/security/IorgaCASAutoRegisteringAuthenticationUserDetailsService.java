...
@Component
public class IorgaCASAutoRegisteringAuthenticationUserDetailsService implements AuthenticationUserDetailsService<CasAssertionAuthenticationToken> {
	// Inspired by fr.ippon.tatami.security.OpenIdAutoRegisteringUserDetailsService
    private final static Logger log = LoggerFactory.getLogger(IorgaCASAutoRegisteringAuthenticationUserDetailsService.class);
    ...
    private static final String FULLNAME_ATTRIBUTE = "displayName";
	...
	public UserDetails loadUserDetails(CasAssertionAuthenticationToken token) throws UsernameNotFoundException {
        ...
        if (login == null) {
            String msg = "CAS response did not contain the user email";
            log.error(msg);
            throw new UsernameNotFoundException(msg);
        }
        if (!login.contains("@")) {
            log.debug("User login {} from CAS response is incorrect.", login);
            throw new UsernameNotFoundException("CAS response did not contains a valid user email");
        }
    ...
    private org.springframework.security.core.userdetails.User getNewlyCreatedUserDetails(CasAssertionAuthenticationToken token) {
        String login = getAttributeValue(token, EMAIL_ATTRIBUTE);
    ...
    private String getAttributeValue(CasAssertionAuthenticationToken token, String name) {
    	return (String) token.getAssertion().getPrincipal().getAttributes().get(name);
    }
}
